...
mod.single <- lm(paste("maxDrift ~ ", piVar), data = isol.log.miss)
res.df <- data.frame(mod.single$residuals, isol.log.miss$Rjb, isol.log.miss$Magnitude)
colnames(res.df) <- c('residuals', 'Rjb', 'Magnitude')
...
ggplot(data = res.df, aes(x=Magnitude, y=residuals)) +
    geom_smooth(method = 'lm') +
    geom_point() +
    ylim(-1, 1) +
    labs(
      title = "Residuals wrt Magnitude",
      x = 'Magnitude',
      y = "Residuals on regression with Pi"
    )

resi.rjb.fit <- lm(residuals ~ Rjb, data = res.df)
summary(resi.rjb.fit)


resi.mag.fit <- lm(residuals ~ Magnitude, data = res.df)
summary(resi.mag.fit)
...
#     )
```
# Sufficiency via linear regression of residuals
...
for (i in seq_along(piList)) {
  ...
  mod.im <- lm(paste("maxDrift ~ ", piVar), data = isol.log.miss)
  res.df <- data.frame(mod.im$residuals, isol.log.miss$Rjb, isol.log.miss$Magnitude)
  colnames(res.df) <- c('residuals', 'Rjb', 'Magnitude')
  
  rjb.lm <- lm(residuals ~ Rjb, data = res.df)
  mag.lm <- lm(residuals ~ Magnitude, data = res.df)
  
  rjb.pval.list[[i]] <- lmp(rjb.lm)
  mag.pval.list[[i]] <- lmp(mag.lm)
}

suf.results.df <- do.call(rbind, Map(data.frame, 
                              PiVar=piList, RjbPval=rjb.pval.list, MagPval=mag.pval.list))
suf.results.df$GM_IM <- varList

ggplot(data=suf.results.df, aes(x = varList, y = RjbPval)) + 
  geom_bar(stat="identity")+
  theme(axis.text.x=element_text(angle=90,hjust=1,vjust=0.5)) + 
  geom_hline(yintercept = 0.05)

ggplot(data=suf.results.df, aes(x = varList, y = MagPval)) + 
  geom_bar(stat="identity")+
  theme(axis.text.x=element_text(angle=90,hjust=1,vjust=0.5)) + 
  geom_hline(yintercept = 0.05)
```
...
Currently, a difference is that we don't know which test Eads uses for her paper. For our purposes, we use the F-statistic.
```{r}
lmp <- function (modelobject) {
    if (class(modelobject) != "lm") stop("Not an object of class 'lm' ")
    f <- summary(modelobject)$fstatistic
...
for (i in seq_along(piList)) {
  piVar <- piList[[i]]
  power.mod <- lm(paste("maxDrift ~ ", piVar), data = isol.log.miss)

  suf.df <- data.frame(power.mod$fitted.values, isol.log.miss$Rjb, isol.log.miss$Magnitude)
  colnames(suf.df) <- c('expected.drift', 'Rjb', 'Magnitude')

  rjb.lm <- lm(expected.drift ~ Rjb, data = suf.df)
  mag.lm <- lm(expected.drift ~ Magnitude, data = suf.df)
